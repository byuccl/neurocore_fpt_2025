...
class GNN(torch.nn.Module):
    def __init__(self, features, classes, width, layers, activation, dropout, pairnorm):
        super().__init__()
        self.width = width
        self.depth = layers
...
def rehydrate(classes, width, layers, act, weights, pairnorm):
    device = torch.device("cpu")
#    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    gnn = GNN(netlist.features, len(classes), width, layers, act, None, pairnorm)
